...
## 8. 교훈 및 안티패턴 (Lessons Learned)

> 실제 발생한 문제와 해결책을 기록합니다.

### 8.1 Case Study: 크로스 도메인 오염 사건 (2025-12-10)

#### 발생 상황
```
jellyfin-domain 작업 → E2E 테스트 실행 → users 테이블 직접 조작
                                        ↓
                              admin 비밀번호 해시 손상
                                        ↓
                              전체 인증 시스템 마비 (401)
...
#### 근본 원인
1. **테스트 환경 미분리**: E2E 테스트가 프로덕션 DB 직접 사용
2. **도메인 경계 침범**: jellyfin 작업이 auth 도메인 데이터 조작
3. **버그 마스킹**: 인증 실패로 기존 catalog 버그가 숨겨짐

#### 해결책
| 문제 | 해결책 | 이슈 |
|------|--------|------|
| 테스트 DB 미분리 | 테스트 전용 PostgreSQL 인스턴스 | #53 |
| 도메인 경계 침범 | DB 테이블 소유권 규칙 추가 | #54 |
| 버그 마스킹 | 스모크 테스트 도입 | #55 |
| 사전 검증 부재 | Pre-Work Checklist | #56 |

### 8.2 안티패턴 목록

| 안티패턴 | 설명 | 올바른 방법 |
|----------|------|-------------|
| **Direct DB Manipulation** | 다른 도메인 테이블 직접 조작 | 해당 도메인 API 통해 요청 |
| **Shared Test DB** | 테스트와 프로덕션 DB 공유 | 격리된 테스트 DB 사용 |
| **Silent Failure Masking** | 상위 에러가 하위 버그 숨김 | 스모크 테스트로 전체 검증 |
| **Scope Creep** | 작업 범위가 도메인 경계 넘음 | Pre-Work Checklist로 검증 |

### 8.3 체크포인트

도메인 작업 시 다음 질문에 "아니오"가 있으면 **중단 후 검토**:

```
□ 수정 파일이 모두 내 도메인 Scope 내인가?
□ DB 조작이 내 도메인 테이블에만 한정되는가?
□ 테스트가 프로덕션 DB를 사용하지 않는가?
□ 작업 전 핵심 기능(로그인, 카탈로그 등)이 정상 동작하는가?
```
...
| 2.1.0 | 2025-12-10 | 교훈 및 안티패턴 섹션 추가 (크로스 도메인 오염 사건 기록) |
